...
const jobObjective = ref("To ensure successful credit analysis is done as requested by the Credit Officers in line with the Bank’s credit policy and within the expected turnaround time, to deliver quality and sustainable risk assets.");
        ...
        <div class="md:w-[60%] space-y-7">
          ...
          <div class="accountabilities md:space-y-5 space-y-3">
            ...
            <ol class="pl-2 space-y-2">
              <li v-for="(accountability, index) in accountabilities" :key="index" class="list-decimal">{{
                accountability }}</li>
            </ol>
          </div>
          <div class="key-performance-indicator md:space-y-5 space-y-3">
            <h2 class="md:text-2xl text-xl font-bold">Key Performance Indicator</h2>
            <div v-for="(kpi, category) in keyPerformanceIndicators" :key="category" class="space-y-2">
              <h3 class="font-semibold">{{ category }}</h3>
              <ul class="space-y-1 pl-2">
                <li v-for="(item, index) in kpi" :key="index" class="list-disc">{{ item }}</li>
              </ul>
            </div>
          </div>
          <div class="required-competencies md:space-y-5 space-y-3">
            <h2 class="md:text-2xl text-xl font-bold">Required Competencies</h2>
            <div v-for="(competency, category) in requiredCompetencies" :key="category" class="space-y-2">
              <h3 class="font-semibold">{{ category }}</h3>
              <ul class="space-y-1 pl-2 list-disc">
                <li v-for="(item, index) in competency" :key="index">{{ item }}</li>
              </ul>
            </div>
          </div>
          <div class="person-specification md:space-y-5 space-y-3">
            <h2 class="md:text-2xl text-xl font-bold">{{ personSpecification.title }}</h2>
            <div class="behavioral-competencies space-y-2">
              <p class="">The behavioural competencies of the job include the ability to:</p>
              <ul class="space-y-1 pl-2 list-disc">
                <li v-for="(item, index) in personSpecification.behavioralCompetencies" :key="index">{{ item }}</li>
              </ul>
            </div>
            <div class="job-profile space-y-2">
              <div class="space-y-4">
                <p v-for="(item, index) in personSpecification.jobProfile" :key="index">{{ item }}</p>
              </div>
            </div>
          </div>
        </div>
        <div class="md:w-[40%]">
          <div class="about-the-job space-y-6">
            <h2 class="md:text-2xl text-xl font-bold">About the Job</h2>
            <div class="md:space-y-6 space-y-4">
              <p class=""><span class="text-black/30">Educational Requirement:</span> {{
                aboutTheJob.educationalRequirement }}</p>
              <p class=""><span class="text-black/30">Experience:</span> {{
                aboutTheJob.experience }}</p>
              <p class=""><span class="text-black/30">Job Title:</span> {{
                aboutTheJob.jobTitle }}</p>
              <p class=""><span class="text-black/30">Grade Band:</span> {{
                aboutTheJob.gradeBand }}</p>
              <p class=""><span class="text-black/30">Age Limit:</span> {{
                aboutTheJob.ageLimit }}</p>
              <p class=""><span class="text-black/30">Unit:</span> {{
                aboutTheJob.unit }}</p>
              <p class=""><span class="text-black/30">Department:</span> {{
                aboutTheJob.department }}</p>
              <p class=""><span class="text-black/30">Team:</span> {{
                aboutTheJob.team }}</p>
              <p class=""><span class="text-black/30">Supervises:</span> {{
                aboutTheJob.supervises }}</p>
              <p class=""><span class="text-black/30">Reports to:</span> {{
                aboutTheJob.reportsTo }}</p>
            </div>
          </div>
          <button
            class="bg-secondary text-white rounded text-[16px] md:text-[20px] md:py-6 py-4 w-full hover:bg-white hover:text-secondary transition-all duration-300 ease-in-out mt-10 border border-white hover:border-secondary">Apply
            Now</button>
        </div>
      </div>
    </div>
